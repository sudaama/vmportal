 ...
 * After this we encounter the  which is a concatenation of HTML code culminating in a string giving the link to the appropriate video. It is intialised to zero.
 ...
 * "Enter" the 'if' statement! When we exceed the maximum number of videos the 'for' loop is broken and the $puzzlesOutString is set to display the remaining set of videos on the last page.
 * The $puzzlesOutString contains the link to the videos stored in the $videos array entered as the last parameter in the displayVideos() method. Information about a particular video is stored in an array of associative arrays and a particular video can be accessed by $videos[$vidCounter + $vidOffset] and the specific info about the link to that video is given by $videos[$vidCounter + $vidOffset]['link'].
 * The $puzzlesOutString is part of the vedicmathcol class and is sent back as list member which will slot into an unordered list              */

class viewPuzzles
	{
		public function displayPuzzles($page, $puzzlesPerPage, $maxPuzzles, $puzzles)
			{
				$puzzlesOffset = $page * $puzzlesPerPage;
				$puzzlesOutString = '';
				for($puzzlesCounter = 0; $puzzlesCounter < $puzzlesPerPage; $puzzlesCounter++) 
					{
						if ($puzzlesCounter + $puzzlesOffset >= $maxPuzzles) 
							{
								break;
							}
						/*		 
						$puzzlesOutString .= '<li>';
						$puzzlesOutString .= '<div class="vedicmathcol">';
						$puzzlesOutString .= '<div>';
						 $puzzlesOutString .= '<p class="puzzletitles">'.$puzzles[$puzzlesCounter + $puzzlesOffset]['title'].'</p>';
						$puzzlesOutString .= '<div><img src="'.$puzzles[$puzzlesCounter + $puzzlesOffset]['graphic']. '" height="200" width="200"></div>';	
						$puzzlesOutString .= '<p class="clearboth">'.$puzzles[$puzzlesCounter + $puzzlesOffset]['description'].'</p>';
						$puzzlesOutString .= '</div>';
						
						
						$puzzlesOutString .= '<div>';
						$puzzlesOutString .= '<p>'.$puzzles[$puzzlesCounter + $puzzlesOffset]['downloadpuz'].'</p>';
						$puzzlesOutString .= '</div>';
						$puzzlesOutString .= '<div>';
						$puzzlesOutString .= '<p>'.$puzzles[$puzzlesCounter + $puzzlesOffset]['downloadpuzsoln'].'</p>';
						$puzzlesOutString .= '</div>';
						...
						*/
						...
						$puzzlesOutString .= '<div class="card mb-4 shadow-sm">'; 
						$puzzlesOutString .= '<div class="card-header">';
						$puzzlesOutString .= '<h6 class="my-0 font-weight-normal">';
						$puzzlesOutString .= $puzzles[$puzzlesCounter + $puzzlesOffset]['title'].'</h6>';
						$puzzlesOutString .= '</div>';
						$puzzlesOutString .= '<div class="card-body">';
						$puzzlesOutString .= '<div class="embed-responsive embed-responsive-16by9">';
						$puzzlesOutString .= '<div><img src="'.$puzzles[$puzzlesCounter + $puzzlesOffset]['graphic']. '"frameborder="0" allowfullscreen ></div>';
						/* $puzzlesOutString .= '<iframe class="embed-responsive-item" src="'.$puzzles[$puzzlesCounter + $puzzlesOffset]['link'].
				...
				return $puzzlesOutString;
		} // end function displayPuzzles()
